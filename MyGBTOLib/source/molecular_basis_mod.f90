...
module molecular_basis_gbl
   ...
   contains
       ...
       !> Writes overlap integrals DISCARD !!!
...
!         endif
   ...
   subroutine print_energy_sorted_orbital_table(this)
      ...
      real(kind=cfp), allocatable :: energies(:)
         ...
         write(level2,'(/,10X,"Continuum orbitals will be ignored.")')
...
!
    ...
    subroutine two_electron_integrals_sparse(this, integral_storage, integral_options)
        ...
        call this % transform_two_indices(no_ao, no_ao, no_mo, Rn, Ri, Rv, Cp, Cj, Cv, &
                                          1, tolerance, skip2ec, max_buffer_size, integral_options % keep_ao_integrals)
        ...
        call this % transform_two_indices(no_mo, no_ao, no_mo, Rn, Ri, Rv, Cp, Cj, Cv, &
                                          2, tolerance, skip2ec, max_buffer_size, integral_options % keep_ao_integrals)
        ...
        call this % finalize_two_electron_integrals_sparse(Rn, Ri, Rv, integral_storage, integral_options, column_descriptor)
    ...
    !>
    subroutine transform_two_indices(this, Nbk, nAO, nMO, Rn, Ri, Rv, Cp, Cj, Cv, step, tolerance, skip2ec, max_buffer_size, &
                                     keep_ao_integrals)
        ...
        do !blocks of AO integrals (iblk_start -> iblk_end) which are gathered sequentially from all tasks
              ...
              if (gather_all_AO_integrals) then
                 ...
                 do
                    ...
                    if (skip2ec) then
                       ...
                       pq_type = this % ao_basis % ordered_pairs(2,pq)
                 ...
                 do iblk = iblk_start, iblk_end
                    ...
                    call this % fetch_atomic_integrals_block(ao_integrals, iblk - 1, rbeg, nnz, &
                                                             Ri, ao_integrals_prefetched % a, skip2ec, &
           ...
           !$omp&          TM_orb, CM_orb, skip2ec, Cp, Cj, Cv, buffptr, ao_integrals, iblk_start, iblk_end) &
           !$omp& private (iblk, rbeg, rcur, rend, ab, cd, a, b, c, d, i, j, blocksym, blockcnt, orb, &
           !$omp&          iRp, Rp, Rj, Vp, Vv, Vj, Wq, Wv, Wj, nnz, ithread, myptr, ierr, mo_mpi_redistribution) &
           ...
           R_loop: do iblk = iblk_start, iblk_end
               ...
               if (step == 1) then
                   ...
                   call this % fetch_atomic_integrals_block(ao_integrals, iblk - 1, rbeg, nnz, Ri, Rv, skip2ec)
        ...
        write (level1, '("Non-zero elements on exit: ",I0)') Rn
    ...
    subroutine sort_intermediate_integrals (Rn, Ri, Rv, n)
        ...
        if (ierr0 /= 0 .or. ierr1 /= 0 .or. ierr2 /= 0 .or. force_heap_sort) then
            write (level1, '("Using serial heap sort")')
            call heap_sort_int_float(Rn, 1, Ri, Rv)
            end_t = omp_get_wtime()
            write (level1, '("Time spent in sort_intermediate_integrals [s]: ",F25.15)') end_t - start_t
            return
        ...
        do i = 2, Rn
            if (Ri(i - 1, 1) > Ri(i, 1)) then
                write(level2, '(I0,",",I0,": ",I0," > ",I0)') i - 1, i, Ri(i - 1, 1), Ri(i, 1)
    ...
    subroutine serial_merge_sorted_int_float (Ai    , Bi    , Ci    , &
        ...
        !ZM protect against failure in extremelly small calculations (may happen
        !if the caller parallel_merge_sorted_int_float is using more threads
        !than the number of elements in Ai and/or Bi).
        if (An == 0 .and. Bn == 0) then
           return
        elseif (An == 0 .and. Bn /= 0) then
           Ci(Cs : Cs-1 + Bn, icol) = Bi(Bs : Be, icol)
           Cf(Cs : Cs-1 + Bn, icol) = Bf(Bs : Be, icol)
           return
        elseif (Bn == 0 .and. An /= 0) then
           Ci(Cs : Cs-1 + An, icol) = Ai(As : Ae, icol)
           Cf(Cs : Cs-1 + An, icol) = Af(As : Ae, icol)
           return
        endif
...
!
         ...
         do int_type=1,d2
            ...
            if (integral_options%max_ijrs_size > 0.0_cfp) then
               ...
               !$OMP SINGLE
               write(level1,'("Size of the ijrs array limited to (Mib): ",f8.3)') integral_options%max_ijrs_size
               !$OMP END SINGLE
            else
...
!            enddo
            ...
            write(level1,'("Total time for (pq|rs) -> (ij|rs) ",f8.3," [s]")') total_ijrs
            write(level1,'("Total time for (ij|rs) -> (ij|kl) ",f8.3," [s]")') total_ijkl
            write(level1,'("(pq|rs) -> (ij|kl) done in ",f8.3," [s]")') total_trmo
            write(level1,'("Number of ijkl integrals: ",i15,1X,f8.3)') &
 ...
 50      call mpi_xermsg ('molecular_orbital_basis_obj', 'read_ijkl_indices', &
    ...
    subroutine construct_canonical_continuum(this, atomic_1el_integral_storage, atomic_2el_integral_storage, &
       ...
       integer :: relative_index !testing
          ...
          !set up from where to read the molecular integrals: file 'moints'  :: THE FILE IS ALREADY OPENED WHEN THIS SUBROUTINE IS
          !CALLED (POSSIBLY COPY STRUCTURE FROM free_scattering_mod.f90 ??
          
          !err = molecular_integral_storage%init(disk='moints')
          !if (err /= 0) then
             !call xermsg ('molecular_basis', 'construct_canonical_continuum', 'Error initializing the integrals storage object.', err, 1)
          !end if
          ...
          call this%read_ijkl_indices(lunit,header%first_record,pos)
       ...
       real(kind=cfp) :: integral_1el, coulomb_integral, exchange_integral
          ...
          do i=begin_index_one,end_index_one
             do j=begin_index_two,end_index_two
                ...
                two_ind(1,1) = i
                two_ind(2,1) = j
                ind(1:1) = this%integral_index(one_elham,two_ind) !1-el integral for orbitals a,b
                ...
                do k=1,this%number_of_functions
                   if (.not. this%is_continuum(k)) then
                      ...
                      ind(1:1) = this%integral_index(two_el_ints,four_ind)
       ...
       real(kind=cfp), allocatable :: overlaps(:,:)
          ...
          do i=begin_index,end_index
             do j=begin_index,end_index
                two_ind(1,1) = i
                two_ind(2,1) = j
                ind(1:1) = this%integral_index(overlap_ints,two_ind) 
