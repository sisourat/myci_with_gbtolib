...
contains
  ...
  function init_boys(this,imax,mmax,step,k)
     use const_gbl, only: boys_f_dprec_asym_thr
     implicit none
  ...
  subroutine eval_taylor_grid(this,Tmax,step)
     ...
     real(kind=cfp) :: T
        ...
        if (.not.(omp_in_parallel())) then
           write(level3,'("Precalculating the Boys function on the grid of ",i6," points and for mmax = ",i3,&
  ...
  subroutine eval_boys_function_taylor(this,boys,d,T,mmax)
     use precisn_gbl
     use utils_gbl, only: xermsg
     use const_gbl, only: fit_order, wp_fit_terms, ep_fit_terms, wp_fit_terms_up, ep_fit_terms_up
     use phys_const_gbl, only: rtpi_half
     implicit none
     class(boys_function_obj) :: this
     ...
     real(kind=cfp) :: T0, delta, two_T, exp_T, test_asym, test_upward
     integer :: i, k, m, f
        ...
        if (cfp .eq. wp) then
           test_upward = cfp_eval_poly_horner(fit_order,T,wp_fit_terms_up)
        elseif (cfp .eq. ep1) then
           test_upward = cfp_eval_poly_horner(fit_order,T,ep_fit_terms_up)
        endif
        ...
        if (cfp .eq. wp) then
           test_asym = cfp_eval_poly_horner(fit_order,T,wp_fit_terms)
        elseif (cfp .eq. ep1) then
           test_asym = cfp_eval_poly_horner(fit_order,T,ep_fit_terms)
        endif
  ...
  subroutine eval_boys_function(this,boys,d,T,mmax)
     use precisn_gbl
     use utils_gbl, only: xermsg
     use const_gbl, only: boys_tol, wp_fit_terms, ep_fit_terms, wp_fit_terms_up, ep_fit_terms_up
     use phys_const_gbl, only: pi
     use special_functions_gbl, only: boys_function_quad
     implicit none
     class(boys_function_obj) :: this
     real(kind=cfp), intent(in) :: T
     integer, intent(in) :: mmax, d
     ...
     integer :: m, i
     real(kind=cfp) :: T_pow, two_T, exp_T, s, s_prev, term, test_upward, test
        ...
        !The estimate for use of the upward recursion differs depending on whether we require full double or quad precision result. Similarly for the asymptotic method below.
        if (cfp .eq. wp) then
           test_upward = cfp_eval_poly_horner(2,T,wp_fit_terms_up)
        elseif (cfp .eq. ep1) then
           test_upward = cfp_eval_poly_horner(2,T,ep_fit_terms_up)
        endif
        ...
        !then we can safely use the asymptotic formula.
        if (cfp .eq. wp) then
           test = cfp_eval_poly_horner(2,T,wp_fit_terms)
        elseif (cfp .eq. ep1) then
           test = cfp_eval_poly_horner(2,T,ep_fit_terms)
        endif
  ...
  function cms_gto_norm(rmat_radius,l,np,alp,ccf,cnorm,norms)
     use special_functions_gbl, only: cfp_gamic, cfp_gamma_fun
     use phys_const_gbl, only: fourpi
     implicit none
  ...
  function CGTO_amplitude(r,l,number_of_primitives,norm,norms,contractions,exponents)
   ...
   subroutine olap_kei_tail(l,np_a,np_b,alp_a,alp_b,ccf_a,ccf_b,cnorm_a, &
      ...
      use special_functions_gbl, only: cfp_gamic
      use phys_const_gbl, only: fourpi
      implicit none
   ...
   !> \f]
   !> where the first integration is only over \f$ r \ge a \f$.
   ...
   subroutine nari_tail(xc,yc,zc,l_a,l_b,np_a,np_b,alp_a,alp_b,ccf_a,ccf_b,cnorm_a,norms_a,cnorm_b,norms_b,rmat_radius,na_tail)
      use special_functions_gbl, only: cfp_gamic, cfp_resh
      use phys_const_gbl, only: fourpi
      implicit none
   ...
   subroutine eri_tail_shell(tgt_prop,tgt_pair,l_a_tgt,l_b_tgt,l_a,l_b,np_a,np_b,&
      ...
      logical :: reallocate
         ...
         reallocate = .false.
         if (allocated(eri_tail_int)) then
            if (size(eri_tail_int) < ind) reallocate = .true.
         endif

         if (.not. allocated(eri_tail_int) .or. reallocate) then
  ...
  function check_real_array_size(a,d)
   ...
   function check_cgto_data(number_of_primitives,l,exponents,contractions,norms,number_of_functions)
      use const_gbl, only: max_contr_len
      implicit none
 ...
 10      call xermsg ('gto_routines', 'write_cgto', 'Error writing the GTO data into the file and position given.', 1, 1)
   ...
   subroutine print_cgto_data(number_of_primitives,l,exponents,contractions,norms,norm,center,non_zero_at_boundary)
      ...
      integer :: n
         ...
         write(level3,'("Contracted spherical GTO shell data:")')
         write(level3,'("L:",i0)') l
         write(level3,'("Center: ",3e20.10)') center
         write(level3,'("Number of primitives:",i0)') number_of_primitives
 
         n = size(exponents)
         if (n > 0) write(level3,'("Exponents:   ",100e20.10)') exponents

         n = size(contractions)
         if (n > 0) write(level3,'("Contractions:",100e20.10)') contractions

         n = size(norms)
         if (n > 0) write(level3,'("Primitive normalization factors:",100e20.10)') norms

         write(level3,'("Contraction normalization factor:",e20.10)') norm
         write(level3,'("Is non-zero at the boundary: ",l1)') non_zero_at_boundary
   ...
   subroutine eval_cgto(r,n_points,number_of_primitives,l,exponents,contractions,norms,norm,center,eval_CGTO_shell)
      use special_functions_gbl, only: cfp_solh
      implicit none
